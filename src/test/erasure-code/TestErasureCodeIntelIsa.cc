...
#include "erasure-code/isa-l/ErasureCodeIntelIsa.h"
#include "common/ceph_argparse.h"
#include "global/global_context.h"
#include "gtest/gtest.h"

template <typename T>
class IntelIsaErasureCodeTest : public ::testing::Test {
 public:
};
...
> IntelTypes;
TYPED_TEST_CASE(IntelIsaErasureCodeTest, IntelTypes);

TYPED_TEST(IntelIsaErasureCodeTest, encode_decode)
{
  TypeParam intelisa;
  map<std::string,std::string> parameters;
  parameters["k"] = "2";
  parameters["m"] = "2";
  intelisa.init(parameters);
...
#define LARGE_ENOUGH 2048
  ...
  EXPECT_EQ(0, intelisa.encode(set<int>(want_to_encode, want_to_encode+4),
                              in,
                              &encoded));
  ...
  // all chunks are available
  {
    ...
    EXPECT_EQ(0, intelisa.decode(set<int>(want_to_decode, want_to_decode+2),
                                encoded,
                                &decoded));
...
TYPED_TEST(IntelIsaErasureCodeTest, minimum_to_decode)
{
  TypeParam intelisa;
  map<std::string,std::string> parameters;
  parameters["k"] = "2";
  ...
  //
  {
    ...
    EXPECT_EQ(0, intelisa.minimum_to_decode(want_to_read,
					    available_chunks,
					    &minimum));
  ...
  //
  {
    ...
    EXPECT_EQ(-EIO, intelisa.minimum_to_decode(want_to_read,
					       available_chunks,
					       &minimum));
  }
  //
...
TEST(IntelIsaErasureCodeTest, encode)
{
  ErasureCodeIntelIsaDefault intelisa;
  map<std::string,std::string> parameters;
  parameters["k"] = "2";
  parameters["m"] = "2";
  intelisa.init(parameters);

  unsigned alignment = intelisa.get_alignment();
  {
    //
...

  {
    ...
    int trail_length = 10;
    in.append(string(alignment + trail_length, 'X'));
    EXPECT_EQ(0, intelisa.encode(want_to_encode, in, &encoded));
 ...
 *   make unittest_erasure_code_intel_isa && 
 *   valgrind --tool=memcheck --leak-check=full \
 *      ./unittest_erasure_code_intel_isa \
